...
c*** computes cubic spline interpolation coefficients for y(x) between grid
c*** points i1 and i2 saving them in q.  The interpolation is continuous with
c*** continuous first and second derivatives.  It agrees exactly with y at
c*** grid points and with the three point first derivitives at both end points
c*** (i1 and i2). x must be monotonic but if two successive values of x are equal
c*** a discontinuity is assumed and separate interpolation is done on each
c*** strictly monotonic segment.  The arrays must be dimensioned at least - x(i2),
c*** y(i2), q(3,i2), and f(3,i2).  f is working storage
c
