...
c kernels kIn in Dziewonski, 1984, or spline kernels, JG, 2002.
      implicit real*8(a-h,o-z)
      common/coeff/coef(4,8,20)
      common/in/p,rdep,itype,imod,itim,idleg,rnorm,numlyr,iso,ider,ish
      common/path$/delt(2,50000,2),nfin(2),kdep
...
c
  111 call addpath_kernel(th,1,1,klay(loc-1),-1)
      call addpath_kernel(th,k,1,klay(loc-1),1)
      goto 100
c
c***  S (Up and Down going wave) ***
c
  112 call addpath_kernel(th,2,1,klay(loc-1),-1)
  113 call addpath_kernel(th,k,1,klay(loc-1),1)
      goto 100
...
C**** p or s ****
  ...
  100 j1=j
   ...
   10 continue
C****  receiver leg ****
      call addpath_kernel(th,k1,1,klay(loc-1),-1)
   20 thetamax=th
      return                  
      end
